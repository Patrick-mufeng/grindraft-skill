...
[Phase 2: 创建目录和骨架文件]
  ↓
[Phase 2.5: 安装 skill 依赖]        ← 新增：Python requests + Node.js puppeteer/canvas
  ↓
...
### Phase 2.5: 安装 skill 依赖

创建骨架后，检测并安装各 skill 所需的运行时依赖。用户可跳过任意项稍后自行安装。

#### 检测流程

```bash
# Python 检测
python --version 2>/dev/null && echo "Python: OK" || echo "Python: MISSING"

# Node.js 检测
node --version 2>/dev/null && echo "Node.js: OK" || echo "Node.js: MISSING"

# pip 检测
pip --version 2>/dev/null && echo "pip: OK" || echo "pip: MISSING"
...
┌─ 可选安装 ─────────────────────────────────────────────┐
│                                                        │
│  [1] pip install requests        (grindraft-illustrate) │
│       → 已安装 ✅                                       │
│                                                        │
│  [2] npm install puppeteer canvas (grindraft-cover)    │
│       → 未安装，是否安装？(Y/n)                         │
│                                                        │
│  输入编号选择，或输入"skip"跳过，或"all"全部安装          │
└────────────────────────────────────────────────────────┘
```

执行逻辑：
1. 先检测 Python + pip 版本，版本达标且 `requests` 未安装 → 提示安装或跳过
2. 再检测 Node.js + npm 版本，版本达标且 `puppeteer`/`canvas` 未安装 → 提示安装或跳过
...
# 验证 puppeteer + canvas
node -e "const p=require('puppeteer');const c=require('canvas');console.log('puppeteer + canvas ✅')"
```

#### 失败处理

- 安装失败（网络问题/权限不足）→ 提示用户手动安装，记录到 state，不阻塞初始化流程
- 用户选择跳过 → 在 `Phase 6` 的入口引导中标注"⚠️ 部分依赖未安装，首次使用相关功能时会再次提示"

#### 依赖状态记录

写入 `.grindraft-state.json` 的 `dependencies` 字段：

```json
{
  "dependencies": {
    "illustrate_python": {"installed": true, "version": "3.12.4"},
    "illustrate_requests": {"installed": true, "version": "2.34.1"},
    "cover_node": {"installed": true, "version": "22.14.0"},
    "cover_puppeteer_canvas": {"installed": false, "skipped_at": "2025-06-17"}
  }
}
```

不阻塞后续流程——依赖缺失只会在对应 skill 首次运行时提示。
...
📦 依赖安装状态：
  配图（grindraft-illustrate）: {Python 3.9+ ✅ / ❌} | requests {已安装 ✅ / 已跳过}
  封面（grindraft-cover）:    {Node.js 18+ ✅ / ❌} | puppeteer+canvas {已安装 ✅ / 已跳过 / 不需要}

{⚠️ 部分依赖未安装——首次使用对应功能时会再次提示。}
{📌 配图需要: pip install requests}
{📌 封面 PNG 需要: npm install puppeteer canvas（HTML 预览无依赖）}
...
📋 自检 Phase 7:
  ...
  □ dependencies → state.json 中 dependencies 字段已填充 ✅
