...
>
> **方法论**（11 阶段闭环）：找热点 → 聊选题 → AI 写初稿 → 去 AI 味 → 配图 → 设计封面 → 排版 HTML → 盲预测 → 发布 → 复盘 → 进化 rubric。
...
把公众号长文写作变成可校准预测循环：**抓热点 → 聊选题 → 写初稿 → 去 AI 味 → 配图 → 做排版 → 盲预测 → 发布 → 复盘 → 进化 rubric**。

本文件是**总协议 + 路由器**。具体每个阶段的工作流在各自的子 skill 里。

> **Claude Code 使用方式**：17 个子 skill 各自独立。安装本插件后，直接用触发词或 skill 名调用——例如说"初始化"会匹配到 `grindraft-init`，说"写文章"匹配到 `grindraft-write`。详见下方路由表。
...
| "复盘 articles/{标题}_{日期}" | 先获取日期 → 计算发布天数 ≥ 3？→ 未到时提示"还差 X 天" |
...
**刷新方式**：调用 `grindraft-status` 的内联逻辑——获取当天日期 → 读 state.json → 扫 articles/ → 渲染看板 → 写入 `STATUS.md`。
...
2. 检查 `articles/` 下有几个文件夹含完整 `## 复盘` 段填了真实数据 → 决定 `mode: cold-start | calibration`
...
├── .env                               # API 配置（YUNWU_API_KEY / YUNWU_BASE_URL）
├── .grindraft-cache/                  # 不入版本控制
│   ├── usage.jsonl                    # 使用日志
│   └── trends-history.jsonl           # 热点去重缓存
├── benchmark.md                       # 基准数据（历史文章导入时填充；含基准指标表 + 历史文章列表 + 对标账号）
├── audience.md                        # 受众画像（grindraft-persona 派生；blind 硬禁读）
├── articles/                          # 每篇文章一个独立文件夹
│   └── {标题}_{YYYY-MM-DD}/             # 文件夹名 = 标题 + 编辑日期
│       ├── draft.md                   # AI 初稿（grindraft-write）
│       ├── final.md                   # 用户修改后的终稿（手动改或 grindraft-humanize）
│       ├── prediction.md              # immutable 预测日志（grindraft-predict）
│       ├── output.html                # 排版后的 HTML（grindraft-format）
│       ├── output-preview.html        # 手机预览版（grindraft-format）
│       ├── cover/                     # 封面（grindraft-cover）
│       │   ├── preview.html
│       │   ├── cover-2x35.png
│       │   └── cover-1x1.png
│       ├── illustrations/             # 正文配图（grindraft-illustrate）
│       │   ├── shot-list.md
│       │   ├── 01-{主题}.png
│       │   └── ...
│       └── retro/                     # 复盘数据（grindraft-retro）
│           └── report.md
├── candidates.md                      # 选题池
...
│   ├── style-precipitation.md         # 风格沉淀协议
│   ├── state-management.md            # .grindraft-state.json 读写约定
│   └── historical-import-protocol.md  # 历史文章导入协议
...
- 新增子 skill → 加 `skills/grindraft-<name>/SKILL.md`
- 新增配图参考 → 加 `skills/grindraft-illustrate/references/<name>.md`
- 修改原则 → 改 `shared-references/<protocol>.md`，所有引用它的 skill 自动跟进
- 修改路由 → 改本文件的"路由表"段
- 子 skill 内部细节 → 直接改对应 `skills/grindraft-*/SKILL.md`
- 新增跨 skill 协议 → 加 `shared-references/<name>.md`，在本文件"文件清单"段注册
