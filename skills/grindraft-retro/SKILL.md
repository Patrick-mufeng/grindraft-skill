...
解析数字 → **立即写入 `retro/<标题>/report.md`**（强制！）。

> ⚠️ `retro/report.md` 是原始数据的唯一权威存储。用户粘贴的原始数字必须一字不改地写入这里。predictions 的 `## 复盘` 段只放分析结果（派生比率 + 验证 + 观察），不重复放原始数据。
...
**读 `retro/report.md` 原始数据**，计算派生比率。派生结果写入 predictions 的 `## 复盘` 段。
...
基准值用粉丝量相近账号的均值（初始默认值，随复盘积累自动更新）。
...
**写入内容**：派生比率 + 逐假设验证 + 提炼观察 + 跨样本检测（Phase 2-5 的输出）。**不写入原始数据**——原始数据在 `retro/report.md` 中。
...
| # | 目标 | 读什么 | 通过条件 |
|---|---|---|---|
| 1 | 预测文件 | predictions/\<file\> | `## 复盘` 段下有实绩数据（非占位"待填"），且 hash 校验通过（预测段未被改） |
| 2 | state.json | .grindraft-state.json | `calibration_samples` 已 +1；`pending_retros` 已剔除本次；`last_retro_at` 已更新 |
...
| 2 | reto/ 目录 | 检查 `retro/<title>/report.md` | 如 Phase 1 写了 reto/report.md，文件存在 |
| 3 | pending_retros 一致性 | 读 state.json 的 `pending_retros` | 本次复盘的文章已从中剔除 |
| 4 | 所有 predictions 完整性 | Glob `predictions/*.md`，读 header | 所有含 `Published at` 的预测文件，要么有 `## 复盘` 实绩数据，要么在 `pending_retros` 中 |
| 5 | calibration_samples 一致性 | 读 state.json | `calibration_samples` = 有实绩复盘的 predictions 数量 |
...
📋 跨文件一致性 Phase 9:
  ...
  □ reto/ → report.md 已写入 ✅（或 "未启用 reto/ 报告"）
  □ pending_retros → 已剔除本次 ✅
  □ predictions 完整性 → 11/11 已发布文章均有复盘或待复盘 ✅
